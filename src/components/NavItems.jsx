...
import list from "../data/navlinks.json";
import AnimatedComponent from "./AnimatedComponent";
import { AnimatePresence } from "framer-motion";
import { fadeInY } from "../utils/animationVariants";

export default function NavItems({ isHeader, ...className }) {
        ...
        <div key={index} className={`${className.list}`}>
          <p
            ...
            className={`relative pb-2  ${className.item}
             ...
             ${
               isHeader
                 ? "list-after line line1 font-medium lg:pl-[1.2rem] lg:text-white"
                 : "font-semibold"
             }`}
            ...
            {(isHeader && activeDropdown === index) || !isHeader ? (
              ...
              >
                {item.links.map((subItem, subIndex) => (
                  <li
                    key={subIndex}
                    className={`py-1 lg:pb-2 lg:pt-[0.3rem] ${
                      active ? "pb-3" : ""
                    }`}
                  >
                    <a
